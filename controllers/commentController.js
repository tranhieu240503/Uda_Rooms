...
const User = require("../models/Users"); // Đảm bảo đường dẫn đúng với cấu trúc dự án
...
const createComment = async (req, res) => {
  ...
  if (!req.body || Object.keys(req.body).length === 0) {
    return res
      .status(400)
      .json({ error: "Không nhận được dữ liệu trong body" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Lỗi tạo bình luận", details: error.message });
  }
};
...
const getCommentsByPost = async (req, res) => {
  try {
    ...
    const comments = await Comment.findAll({
      ...
      include: [
        {
          model: User, // Giả sử bạn có model User
          attributes: ["id", "fullname", "avatar"], // Chỉ lấy các trường cần thiết
        },
      ],
    });
    ...
    return res
      .status(500)
      .json({ error: "Lỗi lấy danh sách bình luận", details: error.message });
  }
};
...
const updateComment = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Lỗi cập nhật bình luận", details: error.message });
  }
};
...
const comment = await Comment.findByPk(idcomment);
    ...
    return res
      .status(500)
      .json({ error: "Lỗi xóa bình luận", details: error.message });
  }
};
module.exports = {
  createComment,
